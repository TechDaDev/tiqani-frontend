/**
 * Proxy: Technician accept request.
 * POST /api/technician/requests/[requestId]/accept/ → Backend POST
 */
...
export async function POST(
  ...
  try {
    const { data, status } = await backendPost<unknown>(
      `/api/technician/requests/${requestId}/accept/`,
      {},
    ...
    return NextResponse.json({ detail: "Failed to accept request." }, { status: 500 });
  }
}
